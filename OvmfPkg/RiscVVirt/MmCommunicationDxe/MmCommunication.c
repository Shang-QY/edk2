  ...
  )
{
  ...
  // SMM Func ID
  CommunicateSmmContext.FuncId = SBI_COVE_SMM_COMMUNICATE;

  // Cookie
  CommunicateSmmContext.Cookie = 0;

  // Copy Communication Payload
  CopyMem ((VOID *)mNsCommBuffMemRegion.VirtualBase, CommBufferVirtual, BufferSize);

  // comm_buffer_address (64-bit physical address)
  CommunicateSmmContext.PayloadAddress = (UINT64)mNsCommBuffMemRegion.PhysicalBase;

  // Call the Standalone MM environment.
  Status = SbiCallCoVESmm(&CommunicateSmmContext);
  switch (Status) {
    case EFI_SUCCESS:
  ...
  )
{
  EFI_STATUS    Status;
  UINT32        MmVersion;
  EFI_RISCV_SMM_CONTEXT      MmVersionContext;

  MmVersionContext.FuncId = SBI_COVE_SMM_VERSION;
  MmVersionContext.Cookie = 0;
  MmVersionContext.PayloadAddress = (UINT64)&MmVersion;

  Status = SbiCallCoVESmm(&MmVersionContext);
